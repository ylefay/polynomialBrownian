...
\mathrm{d}X_u = f_0(X_u)\mathrm{d}u + f_1(X_u)\mathrm{d}\tilde{W}_u,
X_s = y_s
...
See the diffusion term is in $L^2(\mathbb{P})$ and is $\sim \sqrt{h/n_int} = \sqrt(ds)$, over n_int step, sqrt(h).
Equivalently, we can replace
    (\tilde{W}_{i/n_int}-\tilde{W}_{(i-1)/n_int})\sim\mathrm{d}W=((6-12u)I_1/sqrt{6} + W1)du
     by setting du = 1/n_int, u = i/n_int
    1/n_int*((6-12i/n_int)I_1/\sqrt{6} + W1),
...
we replace \tilde{W} by W^n, the piecewise n-th degree polynomial approximation of W.
Or equivalently, we replace \circ\mathrm{d}W_t by \mathrm{d}W^n_t.

We have $||W-W^n||_{L^2(\mathbb{P})} = O(n^{-1/2})$.

Here we use the derivation of polynomials to compute $\mathrm{d}W^n/\mathrm{d}u$ then multiply it by du.
*)
